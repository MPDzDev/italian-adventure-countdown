...
const PirateNegotiationStage = ({ onComplete, onStoryEvent, storyProgress }) => {
  ...
  const [userChoices, setUserChoices] = useState(() => {
    const saved = localStorage.getItem('pizzaioloStage3Choices');
    return saved ? JSON.parse(saved) : {};
  });
  const [isComplete, setIsComplete] = useState(false);
  const [puzzleState, setPuzzleState] = useState(() => {
    const saved = localStorage.getItem('pizzaioloStage3PuzzleState');
    return saved
      ? JSON.parse(saved)
      : {
          sequence: [],
  ...
  // Save choices and puzzle state to localStorage
  useEffect(() => {
  ...
  const RecipeSequencePuzzle = () => {
    ...
    // Add a step to the sequence if not already selected
    const addStep = (step) => {
    ...
    // Validate the sequence and call completeStage if correct
    const checkSequence = () => {
      const isCorrect =
        puzzleState.sequence.length === puzzleState.correctSequence.length &&
        puzzleState.sequence.every((id, index) => id === puzzleState.correctSequence[index]);

      // Increase attempt count and update solved flag
      setPuzzleState(prev => ({
        ...prev,
      ...
      if (isCorrect) {
        ...
        // After a short delay, mark the stage complete so the parent unlocks the next stage
        setTimeout(() => {
          completeStage();
        }, 2000);
      }
    ...
    return (
      <div className="recipe-puzzle">
        ...
        <p>
          For the perfect pizza, the steps must be done in a specific order. Select the steps in the correct sequence:
        </p>
        ...
        {puzzleState.attempts > 0 && !puzzleState.solved && (
          <div className="puzzle-feedback error">That's not quite right. Try again!</div>
        )}
        {puzzleState.solved && (
  ...
  const scenes = [
    {
      ...
      content: (
        <>
          ...
          <p>
            After following clues from the market, you've arrived at a hidden cave near the Leaning Tower.
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>
            The pirates roughly escort you into their hideout. You're brought before their captain—a robust woman with a hat decorated with pasta shapes.
          </p>
          <div className="dialogue">
            <div className="character-avatar">🏴‍☠️</div>
            <div className="dialogue-bubble">
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>
            The pirates warmly welcome you into their hideout. The cave reveals a cozy chamber with maps and utensils among traditional pirate flags.
          </p>
          <div className="dialogue">
            <div className="character-avatar">🏴‍☠️</div>
            <div className="dialogue-bubble">
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>
            Captain Marinara leads you to the pirate galley—a well-equipped kitchen at the back of the cave.
          </p>
          <div className="dialogue">
            <div className="character-avatar">🏴‍☠️</div>
            <div className="dialogue-bubble">
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>Several pirates move toward you, but Antonio steps forward.</p>
          <div className="dialogue">
            <div className="character-avatar">👨‍🍳</div>
            <div className="dialogue-bubble">
              <p className="character-name">Antonio</p>
    ...
    },
    {
      ...
      content: (
        <div className="story-transition">
          ...
          <p>
            As you leave the hideout with{" "}
            {userChoices.pirate_deal === 'accepted'
              ? 'most of the recipes and a new alliance'
              : userChoices.pirate_deal === 'partial'
              ? 'most of the recipes'
              : 'only part of the recipe book'}, you know your next destination.
          </p>
          <div className="dialogue">
            <div className="character-avatar">👨‍🍳</div>
            <div className="dialogue-bubble">
    ...
    // On mount, check if the stage was already completed
    useEffect(() => {
  ...
  // Mark the stage complete, update localStorage, and invoke onComplete to notify the parent
  const completeStage = () => {
    ...
    onComplete(3, {
      ...
      newLocations: [],
      characterRelationships: { antonioBond: 10 }
    });
    // Dispatch a storage event to trigger updates in the parent component
    setTimeout(() => {
