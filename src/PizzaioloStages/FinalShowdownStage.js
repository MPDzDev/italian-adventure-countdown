...
const FinalShowdownStage = ({ onComplete, onStoryEvent, storyProgress }) => {
  ...
  const [userChoices, setUserChoices] = useState(() => {
    const saved = localStorage.getItem('pizzaioloStage5Choices');
    return saved ? JSON.parse(saved) : {};
  });
  const [isComplete, setIsComplete] = useState(false);
  const [finalPuzzleState, setFinalPuzzleState] = useState(() => {
    const saved = localStorage.getItem('pizzaioloStage5PuzzleState');
    return saved
      ? JSON.parse(saved)
      : {
          ingredients: [],
  ...
  // Declare the puzzle component before using it in scenes
  const SecretIngredientPuzzle = () => {
    ...
    return (
      <div className="secret-recipe-puzzle">
              ...
              {finalPuzzleState.ingredients.map(ingredientId => {
                ...
                return (
                  <div key={ingredientId} className="selected-ingredient">
                    <div className="ingredient-icon">{ingredient.icon}</div>
                    <div className="ingredient-name">{ingredient.name}</div>
                    <button className="remove-ingredient" onClick={() => handleRemoveIngredient(ingredientId)}>
                      ✕
                    </button>
                  </div>
                );
        ...
        {finalPuzzleState.ingredients.length > 0 && (
          <button className="test-recipe" onClick={handleTestRecipe}>
            Test Recipe
          </button>
        )}
  ...
  // Memoize scenes to stabilize its reference for effects and dependencies.
  const scenes = useMemo(() => ([
    {
      ...
      content: (
        <>
          ...
          <p>
            After a journey through the rolling Tuscan hills, you arrive at an ancient olive mill. The stone building sits nestled among olive groves, with a weathered pirate flag hanging from one window.
          </p>
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>You knock on the door and call out. After a moment, heavy footsteps approach. The door creaks open to reveal a large man with an impressive flour-dusted beard. He eyes you suspiciously.</p>
          <div className="dialogue">
            <div className="character-avatar">🏴‍☠️</div>
            <div className="dialogue-bubble">
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>A lantern flares to life, revealing a large man with an impressive flour-dusted beard holding a rolling pin like a weapon.</p>
          <div className="dialogue">
            <div className="character-avatar">🏴‍☠️</div>
            <div className="dialogue-bubble">
          ...
          <p>The pirate lowers his rolling pin, looking disappointed.</p>
          <div className="dialogue">
            <div className="character-avatar">🏴‍☠️</div>
            <div className="dialogue-bubble">
              <p className="character-name">Breadbeard</p>
        ...
        <>
          ...
          {isSofiaPresent ? (
            <>
              ...
              <p>After a moment, heavy footsteps approach. The door swings open to reveal a large man with a flour-dusted beard whose stern expression softens when he sees Sofia.</p>
              <div className="dialogue">
                <div className="character-avatar">🏴‍☠️</div>
                <div className="dialogue-bubble">
                  <p className="character-name">Breadbeard</p>
                  <p>"Well, blow me down! A little lass! Reminds me of me own daughter. Come in, come in!"</p>
                </div>
              </div>
              <p>Inside, the mill is cozy with loaves baking in a stone oven.</p>
            </>
          ) : (
            <>
              <p>Without Sofia, you knock on the door instead.</p>
              <p>A large man with a flour-dusted beard opens the door, looking suspicious.</p>
              <div className="dialogue">
                <div className="character-avatar">🏴‍☠️</div>
                <div className="dialogue-bubble">
                  <p className="character-name">Bearded Pirate</p>
              ...
              <p>The pirate opens the door wider.</p>
              <div className="dialogue">
                <div className="character-avatar">🏴‍☠️</div>
                <div className="dialogue-bubble">
                  <p className="character-name">Breadbeard</p>
              ...
              <div className="dialogue">
                ...
                <div className="dialogue-bubble">
                  <p className="character-name">Breadbeard</p>
                  <p>"Wise decision, but ye'll still need to prove yerself worthy of this special recipe!"</p>
                </div>
              </div>
            </>
          )}
      ...
      content: (
        <>
          ...
          <div className="dialogue">
            ...
            <div className="dialogue-bubble">
              <p className="character-name">Breadbeard</p>
              <p>"Here's me challenge: I've been trying to recreate the little lass's special bread, but somethin's always missin'. Show me you know the proper ingredients, and the recipe is yours!"</p>
            </div>
          </div>
          {isSofiaPresent && (
            <div className="dialogue">
      ...
      content: (
        <>
          ...
          <div className="dialogue">
            ...
            <div className="dialogue-bubble">
              <p className="character-name">Breadbeard</p>
              <p>"Before ye go, here's a jar of the wild honey I collected from these hills!"</p>
            </div>
          </div>
        </>
      ),
        ...
        <>
          <div className="story-transition">
            ...
            <div className="dialogue">
              ...
              <div className="dialogue-bubble">
                <p className="character-name">Antonio</p>
                <p>"To celebrate, I'm taking Sofia to the Alpine Splash Waterpark next month. You must join us!"</p>
              </div>
            </div>
            <p>Your Italian adventure has come to an end, but the bonds you've formed will last a lifetime.</p>
          </div>
        </>
      ),
      choices: null
  ...
  // Check if the stage was already completed
  useEffect(() => {
  ...
  // Save user choices and puzzle state when they change
  useEffect(() => {
  ...
  // Wrap completeStage so its identity is stable
  const completeStage = useCallback(() => {
    setIsComplete(true);
    localStorage.setItem('pizzaioloStage5Complete', 'true');
    localStorage.setItem('pizzaioloStage5CompletionTime', new Date().toISOString());
    onComplete(5, {
      choices: userChoices,
      newItems: [],
    ...
    <div className="stage-content story-stage">
      {isComplete ? (
        <div className="success-message">
          ...
          <p>
            You've successfully recovered all of Antonio's recipes, including Sofia's special schiacciatina with its secret wild honey ingredient!
          </p>
          <p>
            The pizzeria is saved, and Antonio has invited you to join their celebration at the Alpine Splash Waterpark.
          </p>
        ...
        <div className={`story-scene ${scenes[currentSceneIndex].background}`}>
          <div className="scene-content">{scenes[currentSceneIndex].content}</div>
          {scenes[currentSceneIndex].puzzle && scenes[currentSceneIndex].puzzle}
          {scenes[currentSceneIndex].choices && (
            <div className="choice-container">
              <h4>What will you do?</h4>
              {scenes[currentSceneIndex].choices.map(choice => (
          ...
          {!scenes[currentSceneIndex].choices &&
            !scenes[currentSceneIndex].puzzle &&
            scenes[currentSceneIndex].id !== 'stage-end' && (
              <button
